...
const VideoPage = {
  ...
  created() {
    // Ensure that the user is allowed to view the video
    if (Cookies.get('Rent_Status') !== 'Rented') {
      this.$router.push('/');
      return;
    }
    ...
    // Retrieve real-time comment count
    db.collection("ElliotComments").onSnapshot(snapshot => {
      this.commentCount = snapshot.size;
    });
    // Check for the video link in the cookie (key: "Film")
    let storedVideoLink = Cookies.get('Film');
    if (storedVideoLink) {
      this.videoLink = storedVideoLink;
    } else {
      // If not found, query Firestore collection "Film link", document with id "Link"
      db.collection("Film link").doc("Link").get()
        .then(doc => {
          if (doc.exists) {
            let link = doc.data().videoLink;
            this.videoLink = link;
            // Cache the retrieved video link in a cookie for 10 days
            Cookies.set('Film', link, { expires: 10 });
          } else {
            console.error("Film link document not found");
          }
        })
        .catch(err => console.error("Error retrieving film link: ", err));
    }
  }
};
